 ...
 * the raw detected boxes, one per measure.
 */
async function detectMeasures(blob: Blob, filename: string): Promise<MeasureBox[]> {
	...
	const res = await fetch(`${measureDetectorUrl.replace(/\/$/, '')}/measures`, {
		method: 'POST',
	...
	const data: DetectorResponse = await res.json();
