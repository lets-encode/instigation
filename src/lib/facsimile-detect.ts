 ...
 */
export async function detectMeasures(
  blob: Blob,
  filename: string,
  detectorUrl: string,
): Promise<MeasureBox[] | null> {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "TimeoutError") {
      throw new Error(
        `The measure detector did not respond within ${DETECTOR_TIMEOUT_MS / 1000}s for ${filename}.`,
      );
    }
  ...
  try {
    ...
    throw new Error(
      `The measure detector returned an invalid response for ${filename}.`,
    );
  }
  if (
    !data ||
    typeof data !== "object" ||
    (data.measures !== undefined && !Array.isArray(data.measures))
  ) {
    throw new Error(
      `The measure detector returned an unexpected response shape for ${filename}.`,
    );
  }
  if (
    (data.measures ?? []).some(
      (measure) => !measure?.bbox || typeof measure.bbox !== "object",
    )
  ) {
    throw new Error(
      `The measure detector returned an unexpected response shape for ${filename}.`,
  ...
  if (invalid.length) {
    throw new Error(
      `The measure detector returned malformed coordinates for ${filename}.`,
    );
  }
  return boxes;
}
