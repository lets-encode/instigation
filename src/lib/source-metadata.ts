...
function optional(tag: string, value: string, depth: number): string {
  return value.trim()
    ? `${indent(depth)}<${tag}>${xmlEscape(value.trim())}</${tag}>\n`
    : "";
}
...
function sourceDescription(
  ...
  const identifier =
    meta.shelfmark.trim() && !meta.repository.trim()
      ? optional("identifier", meta.shelfmark, 4)
      : "";
  ...
  const editionInner =
    ...
    (meta.editionDate.trim()
      ...
      : "");
  const edition = editionInner
    ? `${indent(5)}<edition>${editionInner}</edition>\n`
    : "";
  const pub =
  ...
  const physDesc =
    optional("extent", meta.extent, 5) +
    optional("condition", meta.condition, 5);
  const physLoc = meta.repository.trim()
    ? optional("repository", meta.repository, 5) +
      optional("identifier", meta.shelfmark, 5)
    : "";
  const parts =
    (edition
      ? `${indent(4)}<editionStmt>\n${edition}${indent(4)}</editionStmt>\n`
      : "") +
...
export function buildSourceHead(meta: SourceMetadata): string {
  const people = [
    ...(meta.composer.trim()
      ? [{ name: meta.composer, role: "composer" }]
      : []),
    ...(meta.editor.trim() ? [{ name: meta.editor, role: "editor" }] : []),
    ...(meta.lyricist.trim()
      ? [{ name: meta.lyricist, role: "lyricist" }]
      : []),
  ...
  const manifestation = manifestationListBlock(
    described.identifier + described.parts,
  );
...
export function buildPieceHead(
  ...
  const piecePeople = [
    ...
    ...(piece.lyricist?.trim()
      ? [{ name: piece.lyricist, role: "lyricist" }]
      : []),
  ...
  const sourcePeople = [
    ...(source.composer.trim()
      ? [{ name: source.composer, role: "composer" }]
      : []),
    ...(source.editor.trim() ? [{ name: source.editor, role: "editor" }] : []),
    ...(source.lyricist.trim()
      ? [{ name: source.lyricist, role: "lyricist" }]
      : []),
...
export function parseSourceHead(xml: string): SourceMetadata {
  ...
  const editionInner = tagInner(
    tagInner(manifestation, "editionStmt"),
    "edition",
  );
  meta.editionDate = tagText(editionInner, "date");
  meta.edition = textOf(
    editionInner.replace(/<date\b[^>]*>[\s\S]*?<\/date>/, ""),
  );
  ...
  meta.extraHeadXml = splitAtManifestationSlot(stripped)
    .filter(Boolean)
    .join("\n");
  return meta;
}
