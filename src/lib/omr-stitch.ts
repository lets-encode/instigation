...
export function clefXml(token: string): string {
  ...
  const octave = m[3]
    ? `<clef-octave-change>${Number(m[3])}</clef-octave-change>`
    : "";
...
export function clefTokenOf(clef: string): string | null {
  ...
  const line =
    /<line>\s*(\d+)\s*<\/line>/.exec(clef)?.[1] ?? DEFAULT_LINE[sign] ?? 2;
  const octave = Number(
    /<clef-octave-change>\s*(-?\d+)\s*<\/clef-octave-change>/.exec(clef)?.[1] ??
      0,
  );
  return `${sign}${line}${octave ? (octave > 0 ? `+${octave}` : `${octave}`) : ""}`;
}

const keyXml = (fifths: number): string =>
...
function measuresOf(musicxml: string): string[] {
  ...
  return (
    part.match(/<measure\b[^>]*>[\s\S]*?<\/measure>|<measure\b[^>]*\/>/g) ?? []
  );
}

const inner = (measure: string): string =>
  /<measure\b[^>]*\/>/.test(measure)
    ? ""
    : measure.replace(/^<measure\b[^>]*>/, "").replace(/<\/measure>$/, "");

const element = (xml: string, tag: string): string | null =>
  new RegExp(`<${tag}\\b[^>]*>[\\s\\S]*?</${tag}>|<${tag}\\b[^>]*/>`).exec(
    xml,
  )?.[0] ?? null;
...
function editAttributes(
  ...
  const newBody = attributes
    ? body.replace(attributes, rebuilt)
    : rebuilt + body;
...
export function stitchPage(
  ...
  let time = start.time;
  const opening: PageOpening = {
    clefs: Array.from({ length: parts }, () => null),
    fifths: null,
    time: null,
  };
  ...
  for (const [s, system] of systems.entries()) {
    ...
    const keys = firsts
      ...
      .map(fifthsOf);
    const newFifths =
      keys.length * 2 > transcribed &&
      keys.every((k) => k !== null && k === keys[0]) &&
      keys[0] !== fifths
        ? keys[0]
        : null;
    const times = firsts
      .map((a) => element(a, "time"))
      .filter((t): t is string => !!t)
      .map(meterOf);
    const newTime =
      times.length * 2 > transcribed &&
      times.every((t) => sameMeter(t, times[0])) &&
      !sameMeter(times[0], time)
        ? times[0]
        : null;
    ...
    const signatures: AttributeEdits = opens
      ? { key: keyXml(fifths), time: time ? timeXml(time) : null }
      : {
          key: newFifths !== null ? keyXml(fifths) : null,
          time: newTime ? timeXml(newTime) : null,
        };

    for (let k = 0; k < parts; k++) {
      const measures = [...(staffMeasures[k] ?? [])];
      const inForce = clefs[k];
      const read = measures[0]
        ? element(openingAttributes(measures[0]) ?? "", "clef")
        : null;
  ...
  const partList = perPart
    .map(
      (_, k) =>
        `<score-part id="P${k + 1}"><part-name>Staff ${k + 1}</part-name></score-part>`,
    )
    .join("");
      ...
      (measures, k) =>
        ...
        measures
          .map((m, i) =>
            m.replace(
              /^<measure\b([^>]*?)(\/?)>/,
              (_tag, attrs: string, slash: string) => {
