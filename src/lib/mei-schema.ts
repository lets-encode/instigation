...
import type {
  ErrorDetail,
  RelaxNGValidator,
  XmlDocument as XmlDocumentType,
} from "../../scripts/vendor/libxml2-wasm/index.mjs";

/** The libxml2-wasm entry module, as imported by the consumer. */
export type Libxml2 =
  typeof import("../../scripts/vendor/libxml2-wasm/index.mjs");
...
export const MEI_SCHEMA_URL =
  "https://music-encoding.org/schema/5.1/mei-CMN.rng";
export const MEI_SCHEMA_SHA256 =
...
export async function fetchMeiSchema(): Promise<Uint8Array> {
  ...
  if (!response.ok) {
    throw new Error(
      `Could not download the MEI schema: ${response.status} ${response.statusText} (${MEI_SCHEMA_URL})`,
    );
  }
  const schema = new Uint8Array(await response.arrayBuffer());
  const digest = Array.from(
    new Uint8Array(await crypto.subtle.digest("SHA-256", schema)),
  )
    .map((b) => b.toString(16).padStart(2, "0"))
    .join("");
  if (digest !== MEI_SCHEMA_SHA256) {
    throw new Error(
      `MEI schema integrity check failed (expected ${MEI_SCHEMA_SHA256}, received ${digest})`,
    );
  }
  return schema;
}
...
export function compileMeiSchema(
  lib: Libxml2,
  schema: Uint8Array,
): RelaxNGValidator {
 ...
 */
export function meiValidatorLoader(
  importLib: () => Promise<Libxml2>,
): () => Promise<MeiValidator> {
...
// is needed at all.
export const hasDoctype = (content: string): boolean =>
 ...
 */
export function checkMeiAgainst(
  { lib, validator }: MeiValidator,
  content: string,
): MeiCheck {
  ...
  try {
    ...
    doc = lib.XmlDocument.fromString(content, {
