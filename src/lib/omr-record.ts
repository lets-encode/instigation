...
export function serializeOmrRecord(record: OmrRecord): string {
  ...
  for (const page of record.pages) {
    ...
    for (const staff of page.staves) {
      ...
      const label = staff.label
        ? `      <label>${escape(staff.label)}</label>`
        : "";
...
export function parseOmrRecord(xml: string | null): OmrRecord | null {
  ...
  for (const page of xml.match(/<page n="[\s\S]*?<\/page>/g) ?? []) {
    ...
    for (const [i, from] of starts.entries()) {
      ...
      const score =
        /<score-partwise\b[\s\S]*<\/score-partwise>/.exec(staff)?.[0] ?? null;
      const label = /^<staff\b[^>]*[^/]>\s*<label>([\s\S]*?)<\/label>/.exec(
        staff,
      )?.[1];
      staves.push({
        zone: attr(open, "zone") ?? "",
  ...
  return {
    ...
    staffPipeline: {
      name: attr(pipeline, "name") ?? "",
      version: attr(pipeline, "version") ?? "",
    },
    ocr: {
      engine: attr(ocr, "engine") ?? "",
      version: attr(ocr, "version") ?? "",
