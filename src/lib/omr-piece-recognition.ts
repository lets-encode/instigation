...
import { entryFor, type OmrPageEntry, type OmrRecord, type OmrStaffEntry } from './omr-record.ts';
import { readCachedStaves, staffCacheKey, writeCachedStaves } from './omr-staff-cache.ts';
...
export async function recognisePiece(o: PieceRecognitionOptions): Promise<PieceRecognition> {
	const zoneOf = (n: number, k: number) => `staff-zone-${n}-${k + 1}`;
	const keyOf = (n: number, k: number, pg: PageModel) =>
		staffCacheKey(o.repoId, zoneOf(n, k), pg.staves![k], o.pipeline, STAFF_CROP_MARGIN);

	// What the head's record and this browser already hold.
	const entries: (OmrStaffEntry | null)[][] = o.pages.map((pg, p) =>
		(pg.staves ?? []).map((box, k) => {
			const entry = entryFor(o.existing, zoneOf(p + 1, k), box, o.pipeline, STAFF_CROP_MARGIN);
			return entry?.musicxml ? entry : null;
		})
	);
	const missingKeys = o.pages.flatMap((pg, p) =>
		(pg.staves ?? []).flatMap((_, k) => (entries[p][k] ? [] : [keyOf(p + 1, k, pg)]))
	);
	...
	o.pages.forEach((pg, p) =>
		(pg.staves ?? []).forEach((box, k) => {
			...
			if (hit) entries[p][k] = { zone: zoneOf(p + 1, k), box, label: hit.label, musicxml: hit.musicxml, error: '' };
		})
	);
	const fromCache = cached.size;
	if (fromCache) o.progress(`Continuing with ${fromCache} staves transcribed earlier in this browser`);

	const todo = o.pages.flatMap((pg, p) => (entries[p].some((e) => !e) ? [p] : []));
	if (todo.length) {
	...
	for (let i = 0; i < todo.length; i += PAGES_PER_RUN) {
		...
		const failedNote = failedSoFar ? ` (${failedSoFar} staves failed so far)` : '';
		o.progress(
		...
		const work = await Promise.all(
			batch.map(async (p) => {
				...
				const image = path ? await o.forge.getRepoFileBytes(o.owner, o.repo, path) : null;
				if (!image) throw new Error(`the image of page ${p + 1} (${pg.image}) could not be read.`);
				const missing = (pg.staves ?? []).flatMap((_, k) => (entries[p][k] ? [] : [k]));
				const crops = await cropStaves(image, staffCrops(missing.map((k) => pg.staves![k]), pg));
				return { p, pg, image, missing, crops };
			})
		);
		const xmls = await transcribeStaves(o.client, o.pipeline, work.flatMap((w) => w.crops));
		let at = 0;
		...
		for (const w of work) {
			...
			try {
				...
				systems.forEach((system, s) =>
					system.forEach((box, i) => labels.set(w.pg.staves!.indexOf(box), read[s][i]))
				);
			} catch (e) {
				labels = new Map();
			...
			w.missing.forEach((k, j) => {
				...
				entries[w.p][k] = {
					...
					label,
					musicxml,
					error: musicxml ? '' : 'The staff pipeline returned no transcription.'
				};
	...
	const pages: OmrPageEntry[] = o.pages.flatMap((pg, p) =>
		pg.staves?.length ? [{ n: p + 1, image: pg.image, staves: entries[p].map((e) => e!) }] : []
	);
	return {
		record: {
		...
		failed: pages.flatMap((page) =>
			page.staves.flatMap((s, k) => (s.musicxml ? [] : [{ page: page.n, staff: k + 1 }]))
		),
		fromCache,
		labelErrors
	};
