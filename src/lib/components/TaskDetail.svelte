...
<script lang="ts">
  ...
  // The score viewer, bound so the detail can turn it to a page and read the
  // page it is on.
  let preview = $state<ReturnType<typeof ScorePreview>>();
  /** The score the task addresses, and the page a per-page task opens at. */
  const fragment = $derived(findRow(taskDefs, card.task, "")?.fragment ?? "");
    ...
    <div class="mbody">
      <div class="mpreview">
        {#if fragment}
          ...
          />
        {:else}
          <p class="perr">No score file is recorded for {card.task}.</p>
        {/if}
      </div>
      <div class="mrail">
