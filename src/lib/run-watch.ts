...
export type ProgressUpdate = {
  ...
  detail?: string;
  command?: string;
  /** Sent with `command`: the command resolves to a background result, so it
...
export class WorkflowRunWatch {
  ...
  async tick(): Promise<void> {
    ...
    if (this.state.phase === "searching") {
      ...
      const run = await this.forge.getWorkflowRun(
        this.owner,
        this.repo,
        this.state.run.id,
      );
  ...
  private async find(): Promise<WorkflowRunInfo | null> {
    const runs = await this.forge.listWorkflowRuns(
      this.owner,
      this.repo,
      this.opts.workflow,
      {
    ...
    return (
      runs.find(
        (r) =>
          since === undefined || Date.parse(r.created_at) >= since - 15_000,
      ) ?? null
    );
  }
  ...
  private async announceCurrentStep(runId: number): Promise<void> {
    const jobs = await this.forge.getWorkflowRunJobs(
      this.owner,
      this.repo,
      runId,
    );
    for (const job of jobs) {
      ...
      this.announce({
        step: jobs.length > 1 ? `${job.name}: ${step.name}` : step.name,
      });
      return;
    }
  }
