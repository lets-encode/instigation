...
import { wizard, clearFinishedSetup, COPYRIGHT_ACKNOWLEDGEMENT } from './wizard.svelte.ts';
...
export class CampaignFinisher {
	...
	private async measurePages(): Promise<DetectedPage[]> {
		...
		const name = (i: number) => wizard.images[i].path.split('/').pop() ?? `${i + 1}.jpg`;
		...
		for (const i of wizard.images.keys()) {
			this.log.step(`Detecting measures on page ${i + 1} of ${wizard.images.length}`);
			this.log.detail(name(i));
			const { width, height, boxes, tookMs } = await this.detection.page(i);
			this.log.detail(
				boxes.length ? `${name(i)}: ${boxes.length} measure(s) found` : `${name(i)}: no measures found`
			);
	...
	async run(): Promise<void> {
		...
		if (this.busy) return;
		if (!user || !f) {
			this.error = 'You are no longer signed in. Log in again to finish the campaign.';
			return;
		}
		if (!claim) {
		...
		try {
			...
			const registration = await registerCampaign(claim.name, repo.id, provider.id, claim.token);
			if (registration !== 'ok') {
				this.error =
