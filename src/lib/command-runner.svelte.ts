...
export const viewerId = (): string =>
  auth.user?.id != null ? String(auth.user.id) : "";
...
export class CommandRunner {
  ...
  private logTiming(ok: boolean): void {
    const stepMs = (step: ProgressStep) =>
      step.tookMs ?? (step.endedAt ?? step.startedAt) - step.startedAt;
    const seconds = (ms: number) => Math.round(ms / 100) / 10;
    console.info(
      "[overlay-timing] " +
        JSON.stringify({
          command: this.command,
          steps: this.log.steps.map((step) => ({
            label: step.label,
            s: seconds(stepMs(step)),
          })),
          total_s: seconds(
            this.log.steps.reduce((sum, step) => sum + stepMs(step), 0),
          ),
          ok,
        }),
    );
