...
class PendingVerdictStore {
  ...
  settle(
    ...
    this.entries = this.entries.map((e) =>
      e.id === id ? { ...e, state, message, runFailed } : e,
    );
    if (state === "accepted") {
    ...
    if (state === "accepted")
      setTimeout(() => this.dismiss(id), ACCEPTED_LINGER_MS);
  }
