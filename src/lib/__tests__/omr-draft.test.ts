...
test("each skeleton measure of the page takes the converted measure of the same system and position", () => {
  ...
  assert.match(
    result.mei,
    /<measure xml:id="measure-5" n="5" facs="#zone-2-1">\s*<staff xml:id="[^"]*" n="1">\s*<layer xml:id="[^"]*" n="1"\/>/,
  );
...
test("mismatched systems are filled as far as they go and reported", () => {
  ...
  assert.match(
    result.mei,
    /<measure xml:id="measure-3" n="3" facs="#zone-1-3">\s*<staff xml:id="[^"]*" n="1">\s*<layer xml:id="[^"]*" n="1"\/>/,
  );
  assert.match(
    result.mei,
...
test("a self-closing converted staff does not swallow the staff after it", () => {
  const converted =
    '<mei><section><measure n="1"><staff n="1"/><staff n="2"><layer n="1"><note xml:id="x2" pname="c" oct="4" dur="1"/></layer></staff></measure></section></mei>';
  const result = insertPageDraft(
    skeleton([1]),
    "surface-1",
    converted,
...
test("a clef verovio put inside a tremolo follows the tremolo", () => {
  ...
  assert.match(
    result.mei,
    /<bTrem><note xml:id="t1" pname="c" oct="4" dur="2"\/><\/bTrem><clef shape="F" line="4"\/><\/layer>/,
  );
});
...
test("a key or meter change verovio writes before a measure goes before the skeleton measure it fills", () => {
  ...
  assert.match(
    result.mei,
    /<sb [^>]*\/>\s*<scoreDef><keySig sig="1f"\/><\/scoreDef><measure xml:id="measure-2"/,
  );
});
...
test("a staff the system does not print keeps the clef change verovio placed on it", () => {
  ...
  assert.match(
    result.mei,
    /<staff n="2"><layer n="1"><mRest\/><clef shape="C" line="3"\/><\/layer><\/staff>/,
  );
});
