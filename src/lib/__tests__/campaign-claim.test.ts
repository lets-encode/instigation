...
test("boundaryCheck: only allowed paths, and at least one change", () => {
  assert.equal(
    boundaryCheck(["tracking/lock.csv"], ["tracking/lock.csv"]),
    true,
  );
  assert.equal(
    boundaryCheck(["sources/score.mei"], ["tracking/lock.csv"]),
    false,
  );
...
test("rejects a PR that strays outside lock.csv", () => {
  assert.deepEqual(
    claim({ changedPaths: ["tracking/lock.csv", "sources/score.mei"] }),
    {
      ok: false,
...
test("rejects an unknown task and an invalid kind", () => {
  assert.equal(
    claim({ intent: { task_id: "T9999", subtask_id: "", kind: "encoding" } })
      .reason,
    "unknown_task",
  );
  assert.equal(
    claim({ intent: { task_id: "T0001", subtask_id: "", kind: "review" } })
      .reason,
    "invalid_kind",
  );
});
...
test("rejects a claim whose key does not match its kind", () => {
  ...
  assert.equal(
    claim({
      intent: { task_id: "T0001", subtask_id: "S0001", kind: "encoding" },
    }).reason,
    "invalid_target",
  );
  ...
  assert.equal(
    claim({
      ...
      intent: { task_id: "T0001", subtask_id: "", kind: "validation" },
    }).reason,
    "invalid_target",
  );
});
...
test("encoding claim is rejected when already locked", () => {
  const locks = parseLockCsv(
    LOCK_HEADER + "T0001,,bob,2026-06-25T09:30:00Z,encoding\n",
  );
...
test("a validator with a recorded verdict cannot claim another slot of the subtask", () => {
  ...
  assert.equal(
    claim({ ...base, author: "carol", allowSelfValidation: true }).ok,
    true,
  );
});
