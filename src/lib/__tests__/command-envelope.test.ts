...
test("envelope: round-trips through a PR body", () => {
  const body = appendEnvelopeToPrBody(
    "Reserves T0001/S0001 for validation.",
    ENVELOPE,
  );
...
test("envelopeFromPrBody: rejects invalid envelope fields", () => {
  const body = (envelope: unknown) =>
    `<!-- lets-encode:command ${JSON.stringify(envelope)} -->`;
  ...
  assert.equal(
    envelopeFromPrBody(body({ ...ENVELOPE, timestamp: "not-a-date" })),
    null,
  );
...
test("envelopeColumns: the command columns for a history row, empty without an envelope", () => {
  ...
  assert.deepEqual(envelopeColumns(null), {
    command: "",
    version: "",
    input: "",
  });
