...
test("validationReadyCount: tasks with an undecided, unlocked validation slot", () => {
  ...
  const reviewing = parseLockCsv(
    LOCK_HEADER + "T0004,S0001,9,2026-08-01T10:00:00Z,validation\n",
  );
...
test("loadAllCampaignStats: one unreadable repository is skipped but reported", async () => {
  const f = fakeForge(
    [summary(9101, "ok"), summary(9102, "broken")],
    async (_o, repo) => {
...
test("the campaign slug comes from config campaign.name, not the repo name", async () => {
  ...
  const f = fakeForge(
    [summary(9301, "renamed-repo")],
    async (_o, _r, path) => files[path] ?? null,
  );
  const listing = await loadAllCampaignStats(f, "topic");
  ...
  assert.equal(
    (await loadAllCampaignStats(bare, "topic")).stats[0].name,
    "bare-repo",
  );
});
