...
async function checkBothStages(headXml: string, label: string): Promise<void> {
  ...
  for (const withBreaks of [false, true]) {
    ...
    assert.ok(
      check.ok,
      `${label} at stage ${withBreaks ? "C" : "A"}: ${check.error}`,
    );
  }
}

test(
  "a piece score validates, with the source described in full",
  { skip },
  async () => {
...
test(
  "a piece score validates when nothing about the source is known",
  { skip },
  async () => {
    const head = buildPieceHead(
      { title: "", composer: "" },
...
test(
  "a piece score validates with its creation recorded",
  { skip },
  async () => {
    const head = buildPieceHead(
      {
...
test(
  "a score still validates after a contribution is recorded",
  { skip },
  async () => {
    const contribution = {
      name: "octocat",
    ...
    const bareModel = {
      ...initialFacsimileModel(PAGES),
      headXml: buildSourceHead(source()),
    };
...
test(
  "preserved markup keeps the header valid, wherever it belongs",
  { skip },
  async () => {
...
test(
  "a score rebuilt around a submitted score definition validates",
  { skip },
  async () => {
    ...
    for (const withBreaks of [false, true]) {
      ...
      assert.ok(
        check.ok,
        `set-up facsimile score at stage ${withBreaks ? "C" : "A"}: ${check.error}`,
      );
    }
    for (const pages of [0, 2]) {
      const check = await validateMei(
        buildBlankScoreMei(head, pages, scoreDef),
      );
      assert.ok(
        check.ok,
        `set-up blank score with ${pages} pages: ${check.error}`,
      );
    }
    ...
    assert.ok(
      richCheck.ok,
      `set-up blank score with groups and unpitched staves: ${richCheck.error}`,
    );
  },
);

test(
  "an optimized score with a resting, unprinted staff validates",
  { skip },
  async () => {
    ...
    const zone = (uly: number, pb: boolean) => ({
      box: { ulx: 100, uly, lrx: 900, lry: uly + 300 },
      label: String(uly),
      pb,
      sb: !pb,
      mdiv: false,
