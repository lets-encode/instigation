  ...
  pages: [
    {
      ...
      staves: [
        {
          zone: "staff-zone-3-1",
          box: { ulx: 212, uly: 388, lrx: 1810, lry: 452 },
          label: 'Vl. I & "II"',
          musicxml: MUSICXML,
          error: "",
        },
        {
          zone: "staff-zone-3-2",
          box: { ulx: 212, uly: 540, lrx: 1810, lry: 604 },
          label: "",
          musicxml: null,
        ...
        {
          zone: "staff-zone-3-3",
          box: { ulx: 212, uly: 700, lrx: 1810, lry: 764 },
          label: "Vc.",
          musicxml: null,
          error: "pipeline execution failed",
        },
...
test("a record survives writing and reading", () => {
  ...
  expected.pages[0].staves[0].musicxml =
    '<?xml version="1.0" encoding="UTF-8"?>\n' +
  ...
  assert.match(
    xml,
    /<staff zone="staff-zone-3-2" ulx="212" uly="540" lrx="1810" lry="604" error="pipeline execution failed"\/>/,
  );
});
...
test("an entry is reused only for the same zone, box, pipeline and crop margin", () => {
  ...
  assert.equal(
    entryFor(r, "staff-zone-3-1", box, PIPELINE, 0.9)?.label,
    'Vl. I & "II"',
  );
  assert.equal(
    entryFor(r, "staff-zone-3-1", { ...box, lry: 453 }, PIPELINE, 0.9),
    null,
  );
  assert.equal(
    entryFor(r, "staff-zone-3-1", box, { ...PIPELINE, version: "2" }, 0.9),
    null,
  );
...
test("the record sits next to the score", () => {
  assert.equal(
    omrRecordPath("sources/liszt/score.mei"),
    "sources/liszt/omr.xml",
  );
});
