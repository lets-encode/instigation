...
test("parseFacsimileMei round-trips the model through both active stages", () => {
  ...
  assert.deepEqual(stageA.pages[0].zones[0].box, {
    ulx: 127,
    uly: 259,
    lrx: 1036,
    lry: 804,
...
test("replaceScoreDef swaps every <scoreDef> and leaves the measures alone", () => {
  ...
  assert.equal(
    replaced.replace(/<scoreDef[\s\S]*?<\/scoreDef>/g, ""),
    mei.replace(/<scoreDef[\s\S]*?<\/scoreDef>/g, ""),
  );
});
...
test("a page with no detected measures still emits its surface", () => {
  const mei = buildFacsimileMei(
    initialFacsimileModel([
      { image: "img/01.jpg", width: 800, height: 600, measures: [] },
    ]),
  );
...
test("relinkFacsimileImages retargets each surface and scales it to the image", () => {
  ...
  assert.match(
    relinked,
    /<graphic xml:id="g1" height="1290" target="\.\.\/img\/01\.jpg" width="1000"\/>/,
  );
  assert.match(
    relinked,
    /<zone xml:id="z1" type="measure" ulx="100" uly="129" lrx="500" lry="645"\/>/,
  );
  assert.match(
    relinked,
    /<surface xml:id="s1" ulx="0" uly="0" lrx="1000" lry="1290">/,
  );
...
test("relinkFacsimileImages leaves a self-closing surface as it is", () => {
  ...
  assert.match(
    relinked,
    /<graphic target="\.\.\/img\/02\.jpg" width="50" height="50"\/>/,
  );
});
...
test("relinkFacsimileImages leaves a surface whose graphic declares no size", () => {
  ...
  const relinked = relinkFacsimileImages(mei, [
    { target: "../img/01.jpg", width: 800, height: 600 },
  ]);
...
test("a model without a scoreDef emits the default score definition verbatim", () => {
  for (const mei of [
    buildFacsimileMei(model(), { withBreaks: true }),
...
test("keysig 0 means no accidentals and emits no attribute", () => {
  const mei = buildFacsimileMei({
    ...
    scoreDef: {
      ...DEFAULT_SCORE_DEF,
      staves: [{ clefShape: "F", clefLine: 4, ...PLAIN, label: "" }],
    },
  });
...
test("a symbol signature emits meter.sym instead of the numeric meter", () => {
  ...
  const common = parseScoreDef(
    '<scoreDef meter.sym="common"><staffGrp><staffDef n="1"/></staffGrp></scoreDef>',
  );
...
test("parseScoreDef yields the default without a scoreDef or without staffDefs", () => {
  ...
  assert.deepEqual(
    parseScoreDef("<scoreDef><staffGrp/></scoreDef>"),
    DEFAULT_SCORE_DEF,
  );
...
test("staff groups nest their staves in a symbol-carrying staffGrp and round-trip", () => {
  ...
  const bracketed = {
    ...piano,
    groups: [{ start: 2, end: 3, symbol: "bracket", label: "" }],
  };
...
test("percussion, tablature and octave-displaced staves round-trip", () => {
  ...
  assert.ok(
    mei.includes('lines="6" notationtype="tab.guitar" clef.shape="TAB"'),
  );
  assert.ok(
    mei.includes('lines="7" notationtype="tab.lute.german" meter.count'),
  );
...
test("a system with fewer staff boxes than the definition has staves optimizes the score", () => {
  ...
  const zone = (uly: number, pb: boolean) => ({
    box: { ulx: 0, uly, lrx: 900, lry: uly + 400 },
    label: "",
    pb,
    sb: !pb,
    mdiv: false,
  ...
  const full = buildFacsimileMei(model([20, 150, 520, 650]), {
    withBreaks: true,
    emptyMeasures: true,
  });
  assert.doesNotMatch(full, /optimize/);
  const reduced = buildFacsimileMei(model([20, 150, 520]), {
    withBreaks: true,
    emptyMeasures: true,
  });
