...
test("pre-task ids stay unique across mixed facsimile and physical pieces", () => {
  const config = build({
    pieces: [
      facsimile("piece-01", [1]),
      physical("piece-02", 1),
      facsimile("piece-03", [1]),
    ],
  });
  ...
  assert.deepEqual(
    planned
      .filter((task) => task.locator === "score-setup")
      .map((task) => task.id),
    ["P0002", "P0003", "P0005"],
  );
});
...
test("configToYaml: a physical piece records its page count", () => {
  ...
  assert.ok(
    !configToYaml(build({ pieces: [physical("piece-02")] })).includes("pages:"),
  );
});
...
test("assertSupported: rejects unsupported schema, strategy and piece shapes", () => {
  ...
  assert.throws(
    () => assertSupported(unsupportedPreparation),
    /piece preparation/,
  );
  ...
  const clashing = build({
    pieces: [encoded("piece-01"), encoded("piece-02")],
  });
