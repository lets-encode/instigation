...
const subDef = (task_id: string): TaskRow => ({
  ...task(task_id),
  subtask_id: "S0001",
});
...
function facsimileData(): GraphData {
  return {
    ...
    rows: [
      ...
      state("P0001", "S0001", "completed", "", [
        "pass|carol|2026-07-14T06:44:00Z",
      ]),
      state("P0002", "", "completed", "alice"),
      state("T0001", "", "validation_required", "bob", []),
      state("T0001", "S0001", "validation_required", "", [
        "pass|carol|2026-07-14T09:00:00Z",
      ]),
    ],
...
test("buildGraph assigns concurrent validation locks to separate slots", () => {
  ...
  assert.deepEqual(
    slots.map((slot) => [slot.key, slot.who]),
    [
...
test("buildGraph: nextUp marks the first task the viewer can act on", () => {
  ...
  const d = facsimileData();
  assert.deepEqual(
    buildGraph(d, "you").map((n) => n.nextUp),
    [false, false, false],
  );
  d.locks = [];
  assert.deepEqual(
  ...
  assert.deepEqual(
    buildGraph(d, "bob").map((n) => n.nextUp),
    [false, false, false],
  );
...
test("anonymous viewers can inspect the projection but cannot claim work", () => {
  const nodes = buildGraph(facsimileData(), "");
  assert.equal(
    nodes.some((node) => node.nextUp),
    false,
  );
  assert.equal(
    nodes.flatMap((node) => node.slots).some((slot) => slot.claimable),
    false,
  );
});
...
test("buildGraph: numeric user ids render as logins via the map", () => {
  const data: GraphData = {
    ...
    rows: [
      state("T0001", "", "validation_required", "bob"),
      state("T0001", "S0001", "validation_required", "", [
        "pass|12345|2026-07-14T09:00:00Z",
      ]),
    ],
  ...
  const withLogin = buildGraph(data, "", logins).find(
    (n) => n.task === "T0001",
  );
