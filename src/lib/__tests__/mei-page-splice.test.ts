...
const PAGES: FacsimilePage[] = [
  {
    ...
    width: 400,
    height: 200,
    measures: [box(10, 0), box(10, 1)],
  },
  {
    image: "img/02.jpg",
    width: 400,
    height: 200,
    measures: [box(10, 0), box(10, 1), box(10, 2)],
  },
];
...
const base = buildFacsimileMei(
  { ...initialFacsimileModel(PAGES), headXml: HEAD },
  { withBreaks: true },
);
// A fork that "encoded" every page: each <mRest/> became a note.
const forkAll = base.replace(
  /<mRest\b[^>]*\/>/g,
...
test("splicePage takes only the target page's span from the fork", () => {
  ...
  assert.ok(
    spliced.includes('<pb xml:id="pb-1"') &&
      spliced.includes('<pb xml:id="pb-2"'),
  );
});
...
test("splicePage throws when the page break is missing", () => {
  assert.throws(
    () => splicePage(base, forkAll, "surface-9"),
    /No page break found/,
  );
});
...
test("splicePage keeps a scoreDef the fork added between measures", () => {
  ...
  assert.ok(
    spliced.includes(scoreDef),
    "the inter-measure scoreDef survives the splice",
  );
});
...
test("splicePage drops a measure the fork removed from the page", () => {
  const forkMissing = forkAll.replace(
    /<measure xml:id="measure-5"[\s\S]*?<\/measure>\n?/,
    "",
  );
...
test("splicePage rejects a fork whose page duplicates another page's measure id", () => {
  ...
  const forkDuplicate = forkAll.replace(
    '<measure xml:id="measure-3"',
...
test("the last page's span ends at the enclosing close, nested sections included", () => {
  ...
  const forkNested = blank.replace(
    /<measure xml:id="measure-2"[\s\S]*?<\/measure>/,
    nested,
  );
  ...
  assert.ok(
    spliced.includes("m-b"),
    "the measure after the nested section survives",
  );
