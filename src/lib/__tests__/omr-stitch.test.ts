...
const staff = (attributes: string, measures: string[]) =>
  ...
  measures
    .map(
      (m, i) =>
        `<measure>${i === 0 ? `<attributes><divisions>1</divisions>${attributes}</attributes>` : ""}${m}</measure>`,
    )
    .join("") +
  `</part></score-partwise>`;
const rest =
...
const partOf = (xml: string, k: number) =>
  new RegExp(`<part id="P${k}">([\\s\\S]*?)</part>`).exec(xml)![1];
...
test("a key and time read by most staves are written to every part", () => {
  const page = stitchPage(
    [
      [
        staff(KEY_TIME + G, [rest]),
        staff(KEY_TIME + F, [rest]),
        staff(F, [rest]),
      ],
    ],
    ["G2", "F4", "F4"],
  ...
  for (const k of [1, 2, 3]) {
    const attributes = /<attributes>([\s\S]*?)<\/attributes>/.exec(
      partOf(page.musicxml, k),
    )![1];
    assert.match(
      attributes,
      /^<divisions>1<\/divisions><key><fifths>-1<\/fifths><\/key><time>/,
      `part ${k}`,
    );
    assert.ok(
      attributes.indexOf("<time>") < attributes.indexOf("<clef>"),
      `part ${k}: time before clef`,
    );
  }
});
...
test("a staff with fewer measures than its system is padded; a missing staff is empty measures", () => {
  const page = stitchPage(
    [
      [staff(G, [rest, rest, rest]), null],
      [staff(G, [rest]), staff(F, [rest, rest])],
    ],
    ["G2", "F4"],
    [
      [0, 1],
  ...
  assert.match(
    p2,
    /^<measure number="1"><attributes><divisions>1<\/divisions><key><fifths>0<\/fifths><\/key><clef><sign>F<\/sign><line>4<\/line><\/clef><\/attributes><\/measure><measure number="2"\/>/,
  );
...
test("a short system fills the parts it is placed on and leaves the others empty", () => {
  ...
  assert.match(
    partOf(page.musicxml, 1),
    /^<measure number="1"><attributes><divisions>1<\/divisions><key><fifths>-1<\/fifths><\/key><time>[\s\S]*?<\/time><clef><sign>G<\/sign><line>2<\/line><\/clef><\/attributes><\/measure><measure number="2"\/><measure number="3"[^>]*><print new-system="yes"\/>/,
  );
  // The piano's right hand takes part 2 in both systems, with the key and time.
  assert.match(
    partOf(page.musicxml, 2),
    /<measure number="1"><attributes><divisions>1<\/divisions><key>/,
  );
  assert.equal(
    (partOf(page.musicxml, 3).match(/<measure number="\d+">/g) ?? []).length,
    3,
  );
});
...
test("a staff placed on -1 or beyond the definition is left out", () => {
  const page = stitchPage(
    [[staff(G, [rest]), staff(F, [rest]), staff(F, [rest])]],
    ["G2", "perc"],
    [[0, -1, 5]],
  );
  assert.equal(page.parts, 2);
  assert.match(
    partOf(page.musicxml, 2),
    /^<measure number="1"><attributes><divisions>1<\/divisions><key><fifths>0<\/fifths><\/key><clef><sign>percussion<\/sign><\/clef><\/attributes><\/measure>$/,
  );
});
...
test("the recognised clef is kept with its pitches, and the page opening names it", () => {
  ...
  assert.deepEqual(
    stitchPage([[staff(G, [rest])]], ["G2"], [[0]]).opening.clefs,
    [null],
  );
});
...
test("a repeated system clef is dropped, a different one is kept as a change", () => {
  ...
  assert.match(
    partOf(page.musicxml, 1),
    /<measure number="3"><print new-system="yes"\/><attributes><divisions>1<\/divisions><clef><sign>C</,
  );
});

test("a staff without a clef continues in the clef in force", () => {
  const page = stitchPage(
    [[staff(C3, [rest])], [staff("", [rest])], [staff(C3, [rest])]],
    ["F4"],
    [[0], [0], [0]],
  );
...
test("a clef change inside a staff is in force at the next system", () => {
  ...
  const page = stitchPage(
    [[staff(G, [rest, change])], [staff(F, [rest])]],
    ["G2"],
    [[0], [0]],
  );
...
test("an octave clef is told apart from its plain form", () => {
  ...
  assert.deepEqual(
    stitchPage([[staff(G8, [rest])]], ["G2-1"], [[0]]).opening.clefs,
    [null],
  );
  assert.deepEqual(
    stitchPage([[staff(G8, [rest])]], ["G2"], [[0]]).opening.clefs,
    ["G2-1"],
  );
});

const key = (fifths: number) => `<key><fifths>${fifths}</fifths></key>`;
const time = (beats: number, type: number) =>
...
test("key and meter change only on a majority of the staves, none reading another", () => {
  ...
  for (const k of [1, 2]) {
    ...
    assert.deepEqual(
      part.match(/<fifths>-?\d+<\/fifths>/g),
      ["<fifths>-2</fifths>", "<fifths>1</fifths>"],
      `part ${k}`,
    );
    assert.deepEqual(
      part.match(/<beats>\d+<\/beats>/g),
      ["<beats>3</beats>", "<beats>2</beats>"],
      `part ${k}`,
    );
    assert.match(
      part,
      /<measure number="3"[^>]*>(<print new-system="yes"\/>)?<attributes>(<divisions>1<\/divisions>)?<key><fifths>1<\/fifths><\/key><\/attributes>/,
    );
  }
});
...
test("a key the first system changes is reported for the page opening", () => {
  const page = stitchPage(
    [[staff(key(3) + G, [rest]), staff(key(3) + F, [rest])]],
    ["G2", "F4"],
    [[0, 1]],
    START,
  );
  ...
  const three = [
    [staff(key(3) + G, [rest]), staff(key(3) + G, [rest]), staff(F, [rest])],
  ];
  assert.equal(
    stitchPage(three, ["G2", "G2", "F4"], [[0, 1, 2]], START).opening.fifths,
    3,
  );
  assert.equal(
    stitchPage(
      [[staff(key(3) + G, [rest]), staff(F, [rest])]],
      ["G2", "F4"],
      [[0, 1]],
      START,
    ).opening.fifths,
...
test("a page whose first system has no transcription opens at the next system", () => {
  const page = stitchPage(
    [
      [null, null],
      [staff(key(-2) + G, [rest]), staff(F, [rest])],
    ],
    ["G2", "G2"],
    [
      [0, 1],
  ...
  assert.match(
    p2,
    /^<measure number="1"><attributes><divisions>1<\/divisions><key><fifths>-2<\/fifths><\/key><time>[\s\S]*?<\/time><clef><sign>F</,
  );
});
...
test("a meter read without the symbol of the common time in force is no change", () => {
  ...
  const page = stitchPage(
    [[staff(time(4, 4) + G, [rest]), staff(time(4, 4) + F, [rest])]],
    ["G2", "F4"],
    [[0, 1]],
    common,
  );
