...
function fakeFetch(routes: Record<string, (init?: RequestInit) => Response>) {
  ...
  const doFetch = async (
    ...
    const route =
      routes[`${method} ${url}`] ?? routes[`${method} ${url.split("?")[0]}`];
    if (!route)
      return Response.json(
        { error: `unrouted ${method} ${url}` },
        { status: 404 },
      );
    return route(init);
  };
...
test("requirePipelines names the pipelines the service no longer offers", async () => {
  ...
  await client.requirePipelines([
    { name: "dvorak-ola", version: "2.0-2025-03-09" },
  ]);
...
test("upload asks for presigned URLs, then puts each file through the blob relay", async () => {
  const { seen, fetch } = fakeFetch({
    [`POST ${BASE}/api/musicorpus-pages/p1/file-urls`]: () =>
      Response.json({
        put: {
          "Staves/1/image.jpg":
            "https://quest.example/bucket/p1/Staves/1/image.jpg?sig=a",
          "Staves/2/image.jpg":
            "https://quest.example/bucket/p1/Staves/2/image.jpg?sig=b",
        },
  ...
  assert.deepEqual(JSON.parse(seen[0].body!), {
    put: ["Staves/1/image.jpg", "Staves/2/image.jpg"],
  });
  assert.equal(
    seen[1].url,
...
test("download fetches each presigned URL through the blob relay", async () => {
  const { fetch } = fakeFetch({
    [`POST ${BASE}/api/musicorpus-pages/p1/file-urls`]: () =>
      Response.json({
        get: { "layout.json": "https://quest.example/b/layout.json?sig=c" },
      }),
    [`GET ${BASE}/blob`]: () =>
      new Response('{"annotations":[]}', {
...
test("a call the broker rate limit refused is retried after a back-off; a service 429 is not", async () => {
  ...
  const { seen, fetch } = fakeFetch({
    [`GET ${BASE}/blob`]: () =>
      refusals-- > 0 ? brokerLimited() : new Response("bytes"),
    [`POST ${BASE}/api/musicorpus-pages/p1/file-urls`]: () =>
      Response.json({
        get: { "a.json": "https://quest.example/b/a.json?sig=d" },
      }),
    [`GET ${BASE}/api/pipelines`]: () =>
      Response.json(
        { error: "slow down" },
        { status: 429, headers: { "X-Lets-Encode-Upstream": "musibot" } },
      ),
  });
  ...
  await assert.rejects(
    client.pipelines(),
    (e: unknown) => e instanceof OmrError && e.status === 429,
  );
  assert.deepEqual(waits, [1000, 2000]);

  refusals = 10;
  await assert.rejects(
    client.download("p1", ["a.json"]),
    /rate limit exceeded/,
  );
...
test("run gives up on an execution that never settles", async () => {
  ...
  try {
    await assert.rejects(
      client.run("p1", { name: "mzk-staff", version: "1" }, [
        "Staves/1/image.jpg",
      ]),
...
test("withPage deletes the page even when the work fails", async () => {
  const { seen, fetch } = fakeFetch({
    [`POST ${BASE}/api/musicorpus-pages`]: () =>
      Response.json({ page_id: "p9", executions: [] }, { status: 201 }),
    [`DELETE ${BASE}/api/musicorpus-pages/p9`]: () =>
...
test("a failing relay status carries the relay's message and the status", async () => {
  ...
  await assert.rejects(client.createPage(), (err: unknown) => {
    ...
    assert.match(
      err.message,
      /returned 503 .*: OMR is not configured on this instance/,
    );
    return true;
  });
});
