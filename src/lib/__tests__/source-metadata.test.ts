...
test("keeps the composer and editor distinct from other contributors", () => {
  ...
  assert.match(
    xml,
    /<persName[^>]*role="composer"[^>]*>L\. van Beethoven<\/persName>/,
  );
...
test("the edition's year is a date within the edition", () => {
  ...
  assert.match(
    xml,
    /<edition[^>]*>2nd revised edition <date[^>]*>1854<\/date><\/edition>/,
  );
  const parsed = parseSourceHead(xml);
  ...
  const dateOnly = buildSourceHead({
    ...emptySourceMetadata(),
    editionDate: "1854",
  });
...
test("describes the source in the manifestation, not the file", () => {
  const xml = buildSourceHead(filled());
  const manifestation =
    /<manifestation\b[\s\S]*<\/manifestation>/.exec(xml)?.[0] ?? "";
  assert.match(
    manifestation,
