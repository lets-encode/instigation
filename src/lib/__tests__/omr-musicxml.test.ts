...
test("readAttributes: clef, key and time of the first attributes block", () => {
  ...
  assert.deepEqual(
    readAttributes(staff("<clef><sign>G</sign><line>2</line></clef>")),
    {
  ...
  assert.deepEqual(readAttributes("<score-partwise/>"), {
    clef: null,
    fifths: null,
    time: null,
  });
...
test("proposeScoreDef: each staff its own clef; key and meter from the first staff carrying them", () => {
  const proposal = proposeScoreDef([
    staff(
      "<key><fifths>2</fifths></key><time><beats>3</beats><beat-type>8</beat-type></time><clef><sign>G</sign><line>2</line></clef>",
    ),
...
test("proposeScoreDef: a failed staff keeps the treble clef; a symbol meter is carried; empty input is one staff", () => {
  const proposal = proposeScoreDef([
    null,
    staff(
      '<time symbol="cut"><beats>2</beats><beat-type>2</beat-type></time><clef><sign>G</sign><line>2</line><clef-octave-change>-1</clef-octave-change></clef>',
    ),
  ]);
  assert.deepEqual(
    proposal.staves.map((s) => [s.clefShape, s.clefDis, s.clefDisPlace]),
    [
      ["G", "", ""],
      ["G", "8", "below"],
    ],
...
test("proposeScoreDef: signatures are read from the given sources before the clef system", () => {
  const proposal = proposeScoreDef(
    ...
    [
      staff(
        "<key><fifths>-2</fifths></key><time><beats>6</beats><beat-type>8</beat-type></time>" +
          F,
      ),
    ],
  );
  assert.deepEqual(
    proposal.staves.map((s) => s.clefShape),
    ["G", "F"],
  );
...
const opening = (clef: string) =>
  `<attributes><divisions>1</divisions>${clef}</attributes>`;
...
const pitches = (xml: string) =>
  ...
  ].map((m) => `${m[1]}${m[2] ? (Number(m[2]) > 0 ? "#" : "b") : ""}${m[3]}`);
...
test("correctClef moves the notes so they keep their lines and spaces", () => {
  ...
  const { musicxml, state } = correctClef(
    doc(opening(G) + pitch("E", 4) + pitch("A", 5)),
    "G2",
    "F4",
    "misread",
  );
...
test("correctClef stops at the first clef read as something else, for good", () => {
  ...
  assert.deepEqual(correctClef(staff, "G2", "F4", "ended"), {
    musicxml: staff,
    state: "ended",
  });
  assert.deepEqual(correctClef(staff, "G2", "perc", "misread"), {
    musicxml: staff,
    state: "misread",
  });
});
...
test("correctClef moves a staff without a clef, which continues in the misread one", () => {
  ...
  assert.deepEqual(
    pitches(
      correctClef(doc(opening(F) + pitch("G", 2)), "F4", "G2-1", "misread")
        .musicxml,
    ),
    ["E3"],
  );
...
test("correctClef keeps the alter of a note tied from the measure before", () => {
  ...
  const { musicxml } = correctClef(
    doc(
      opening(G) +
        pitch("C", 5, "<alter>1</alter><accidental>sharp</accidental>"),
      pitch("C", 5, tieStop),
    ),
    "G2",
    "C3",
...
test("correctClef leaves a clef read as the corrected one in force, and replaces a clef of the same lines", () => {
  ...
  const { musicxml, state } = correctClef(
    doc(
      opening(G) + pitch("E", 4),
      bass + pitch("G", 2),
      `<attributes>${G}</attributes>` + pitch("E", 4),
    ),
    "G2",
    "F4",
    "misread",
  ...
  const after = correctClef(
    doc(opening(G) + pitch("E", 4), bass + pitch("G", 2)),
    "G2",
    "F4",
    "misread",
  );
  ...
  assert.deepEqual(
    pitches(correctClef(doc(pitch("G", 2)), "G2", "F4", after.state).musicxml),
    ["G2"],
  );
  // F3 and C5 put the same pitch on the bottom line: only the clef changes.
  const F3 = "<clef><sign>F</sign><line>3</line></clef>";
  const same = correctClef(
    doc(opening(F3) + pitch("B", 2, "<alter>-1</alter>")),
    "F3",
    "C5",
    "misread",
  ).musicxml;
