...
function pieceScore(): string {
  const head = buildPieceHead(
    { title: "Sonata I", composer: "L. van Beethoven" },
    emptySourceMetadata(),
    {
      creator: "owner",
      date: "2026-08-09",
...
test("records the change, the contributor and the application", () => {
  ...
  assert.match(
    out,
    /<change n="2" isodate="2026-08-10">[\s\S]*?<persName>octocat<\/persName>[\s\S]*?<p>Encode page 3\.<\/p>/,
  );
  assert.match(out, /<persName role="contributor">octocat<\/persName>/);
  assert.match(
    out,
    /<application xml:id="app-mei-friend">\s*<name>mei-friend<\/name>/,
  );
});

test("numbers changes after the existing ones", () => {
  const twice = recordContribution(
    recordContribution(pieceScore(), CONTRIBUTION),
    {
      ...CONTRIBUTION,
      name: "hubot",
      message: "Fix slurs.",
...
test("creates respStmt, encodingDesc and revisionDesc when the header lacks them", () => {
  ...
  assert.match(
    out,
    /<respStmt>\s*<persName role="contributor">octocat<\/persName>\s*<\/respStmt>/,
  );
...
test("a name carrying replacement-string patterns is inserted literally", () => {
  ...
  assert.match(
    out,
    /<persName role="contributor">\$' \$&amp; o'brien<\/persName>/,
  );
...
test("returns a score with no header unchanged", () => {
  assert.equal(
    recordContribution("<mei><music/></mei>", CONTRIBUTION),
    "<mei><music/></mei>",
  );
});
