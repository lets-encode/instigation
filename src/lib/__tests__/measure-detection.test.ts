...
test("the detector copy is downscaled and results are scaled to the full page", async (t) => {
  ...
  const session = startDetection(
    [page("sources/img/01.jpg", "full-size-bytes")],
    "https://d.example",
    {
      ...options,
...
test("identical page content is sent to the detector once", async (t) => {
  ...
  const session = startDetection(
    [
      page("sources/img/01.jpg", "twin-bytes"),
      page("sources/img/02.jpg", "twin-bytes"),
    ],
    "https://d.example",
    options,
  );
...
test("at most two requests are in flight at once", async (t) => {
  ...
  const images = ["a", "b", "c", "d"].map((content, i) =>
    page(`sources/img/0${i + 1}.jpg`, `flight-${content}`),
  );
  ...
  let done = false;
  const all = Promise.all(images.map((_, i) => session.page(i))).then(
    () => (done = true),
  );
...
test("cancel stops pages that have not started; in-flight ones finish", async (t) => {
  ...
  const images = ["a", "b", "c"].map((content, i) =>
    page(`sources/img/0${i + 1}.jpg`, `cancel-${content}`),
  );
  const session = startDetection(images, "https://d.example", options);
...
test("a detector failure (HTTP 500) is not cached; a genuine empty page is", async (t) => {
  ...
  assert.equal(
    ...
    2,
    "the empty detection is served from the cache",
  );
  session.cancel();
  again.cancel();
});
