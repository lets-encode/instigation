...
test('transcribeStaves: one page, every crop uploaded, one run per staff, null for a failed staff', async () => {
	...
	const client = {
		...
		async run(pageId: string, _pipeline: unknown, input: string[]): Promise<OmrExecution> {
		...
		async download(_pageId: string, paths: string[]) {
			...
			return Object.fromEntries(paths.map((path) => [path, new Blob([`<xml>${path}</xml>`])]));
		}
	} as unknown as OmrClient;

	const xmls = await transcribeStaves(client, pipeline, [new Blob(['a']), new Blob(['b']), new Blob(['c'])]);
	assert.deepEqual(xmls, ['<xml>Staves/1/transcription.musicxml</xml>', null, '<xml>Staves/3/transcription.musicxml</xml>']);
