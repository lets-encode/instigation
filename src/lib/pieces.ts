...
export function createPiece(
  pieces: Piece[],
  kind: PieceKind = "facsimile",
): Piece {
  return {
    id: nextPieceId(pieces),
    kind,
    zones: [],
    meta: emptySourceMetadata(),
  };
}
 ...
 */
export function createEncodedPiece(
  pieces: Piece[],
  encodingName: string,
): Piece {
 ...
 */
export function overlappingPiece(
  pieces: Piece[],
  index: number,
  zone: PieceZone,
): number {
  return pieces.findIndex(
    (piece, p) =>
      p !== index && piece.zones.some((other) => zonesOverlap(other, zone)),
  );
}
...
export function coverPages(
  ...
  if (!piece || piece.kind !== "facsimile")
    return { blocked: [], blockers: [] };
  const blocked: number[] = [];
  const blockers = new Set<string>();
  for (const surface of surfaces) {
    pieces.forEach((other, p) => {
      if (p === index || !other.zones.some((zone) => zone.surface === surface))
        return;
      if (!blocked.includes(surface)) blocked.push(surface);
      blockers.add(other.meta.title.trim() || other.id);
    });
  }
  const free = surfaces.filter(
    (surface) => !blocked.includes(surface) && sizes[surface],
  );
  piece.zones = [
  ...
  return pieces.map((piece, p) => {
    ...
    const pages = covered
      ...
      .map((surface) => {
        ...
        const measures = page.boxes.filter(
          (_box, b) => owners[surface][b] === p,
        );
        return { ...page, boxes: measures, measures };
      });
    const measuredSurfaces = pages.flatMap((page, i) =>
      page.measures.length ? [i + 1] : [],
    );
