...
export interface OcrReader {
  ...
  read(
    image: ImageBitmap,
    options?: { region?: OcrBox; scale?: number; sparse?: boolean },
  ): Promise<OcrLine[]>;
  close(): Promise<void>;
}
 ...
 * no label.
 */
export async function readStaffLabels(
  image: Blob,
  systems: MeasureBox[][],
): Promise<string[][]> {
  const margins = systems.map((system) =>
    system.length ? Math.min(...system.map((box) => box.ulx)) - 2 : 0,
  );
  ...
  try {
    const lines = await reader.read(bitmap, {
      region: [0, 0, right, bitmap.height],
      scale: 3,
      sparse: true,
    });
    ...
    for (const word of lines.flatMap((line) => line.words)) {
      ...
      if (s < 0 || word.box[2] > margins[s] || !/\p{L}/u.test(word.text))
        continue;
      let best = -1;
