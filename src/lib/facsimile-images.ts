...
export function resolveRepoRelativeTarget(
  fragment: string,
  target: string,
): string | null {
  if (
...
export async function resolveFacsimileImageUrls(
  ...
  const paths = targets.map((target) =>
    resolveRepoRelativeTarget(fragment, target),
  );
  const directories = [
    ...new Set(
      paths.filter((path): path is string => path !== null).map(dirname),
    ),
  ];
    ...
    await Promise.all(
      directories.map(
        async (directory) =>
          [
            directory,
            await forge.getDirDownloadUrls(owner, repo, directory, ref),
          ] as const,
      ),
    ),
  );
