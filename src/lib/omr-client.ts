...
export function createOmrClient(
  brokerUrl: string,
  ...
  const executionTimeoutMs =
    options.executionTimeoutMs ?? DEFAULTS.executionTimeoutMs;
  ...
  async function call(
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      const brokerLimited =
        res.status === 429 &&
        res.headers.get("X-Lets-Encode-Upstream") === "broker";
  ...
  const blobUrl = (presigned: string) =>
    `${base}/blob?url=${encodeURIComponent(presigned)}`;
  ...
  async function fileUrls(
    ...
    }>("POST", `musicorpus-pages/${pageId}/file-urls`, request);
    return { put: urls.put ?? {}, get: urls.get ?? {} };
  }

  const client: OmrClient = {
    async pipelines() {
      const listing = await api<{ pipelines?: OmrPipelineEntry[] }>(
        "GET",
        "pipelines",
      );
    ...
    async requirePipelines(required) {
      ...
      const missing = required.filter(
        (p) =>
          !offered.some((o) => o.name === p.name && o.version === p.version),
      );
      if (missing.length) {
        throw new OmrError(
    ...
    async createPage() {
      ...
      if (!page.page_id)
        throw new OmrError("The OMR service created a page without an id.", 0);
      return page.page_id;
    },
    ...
    async upload(pageId, files) {
      ...
      for (const path of paths) {
        ...
        if (!url)
          throw new OmrError(
            `The OMR service gave no upload URL for ${path}.`,
            0,
          );
        await call("PUT", blobUrl(url), {
          headers: {
            "content-type": files[path].type || "application/octet-stream",
          },
          body: files[path],
        });
      }
    ...
    async download(pageId, paths) {
      ...
      for (const path of paths) {
        ...
        if (!url)
          throw new OmrError(
            `The OMR service gave no download URL for ${path}.`,
            0,
          );
    ...
    async run(pageId, pipeline, input, parameters = {}) {
      const started = await api<OmrExecution>(
        "POST",
