...
export function readyCount(
  d: Pick<GraphData, "taskDefs" | "rows" | "locks">,
): number {
...
export function validationReadyCount(
  ...
  const hasOpenSlot = (r: StateRow): boolean => {
    const finals = d.validationColumns.filter((c) =>
      isFinalValidation(r[c] ?? ""),
    ).length;
    const held = d.locks.filter(
      (l) =>
        l.task_id === r.task_id &&
        l.subtask_id === r.subtask_id &&
        l.kind === "validation",
    ).length;
...
export function nextTask(
  ...
  const describe = (
    task: string,
  ): Omit<NextTask, "action" | "subtask" | "kind"> => {
  ...
  if (mine) {
    ...
    if (slot)
      return { ...base, action: "review", subtask: slot.sub, kind: "review" };
    // Held work: the lock says whether the viewer encodes or validates.
    const held = stats.locks.find(
      (l) => l.task_id === mine.task && l.user_id === viewer,
    );
    return {
      ...base,
      action: "continue",
  ...
  const open = nodes.find(
    (n) =>
      ...
      (n.statusKey === "validation_required" &&
        n.slots.some((s) => s.key === "open")),
  );
  if (!open) return null;
...
export function myTasksIn(stats: CampaignStats, viewer: string): MyTask[] {
  ...
  const base = (task: string): Omit<MyTask, "group"> => {
    const def = findRow(stats.taskDefs, task, "");
    const subRows = stats.rows.filter(
      (r) => r.task_id === task && r.subtask_id !== "",
    );
    const cells = subRows.flatMap((r) =>
...
export function commentsOnMyWork(
  stats: CampaignStats,
  viewer: string,
): FeedComment[] {
  if (!viewer) return [];
  const myTasks = new Set(
    stats.rows
      .filter((r) => r.subtask_id === "" && r.encoder === viewer)
      .map((r) => r.task_id),
  );
