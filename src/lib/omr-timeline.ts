...
const plainClef = (token: string | null): string | null =>
  token?.replace(/[+-]\d$/, "") ?? null;
    ...
    systems.forEach((system, s) => {
      const placed = suggestStaffAssignment(
        ...
        reference,
        {
          ...
          clefs: clefs.map((clef, k) => {
            ...
            return correction && correcting.get(k + 1) === "misread"
              ? plainClef(correction.read)
              : clef;
          }),
          previous,
      ...
      transcriptions.push(
        entries[s].map((entry, i) => {
          ...
          if (xml && correction) {
            const result = correctClef(
              xml,
              correction.read,
              correction.corrected,
              correcting.get(correction.staff)!,
            );
            xml = result.musicxml;
          ...
          if (last && placed[i] > 0)
            clefs[placed[i] - 1] = plainClef(clefTokenOf(last));
          return xml;
        }),
      );
 ...
 */
export function openingClefs(
  pages: PageStaves[],
  staffCount: number,
): (string | null)[] {
  const clefs: (string | null)[] = Array.from(
    { length: staffCount },
    () => null,
  );
  for (const staves of pages) {
    staves.entries.forEach((system, s) =>
      system.forEach((entry, i) => {
        ...
        const clef = entry?.musicxml
          ? /<clef\b[^>]*>[\s\S]*?<\/clef>/.exec(entry.musicxml)?.[0]
          : undefined;
        if (n > 0 && n <= staffCount && clefs[n - 1] === null && clef)
          clefs[n - 1] = clefTokenOf(clef);
      }),
    );
  }
  return clefs;
