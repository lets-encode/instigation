  ...
  try {
    ...
    for (const n of wanted) {
      ...
      const viewport = page.getViewport({
        scale: Math.min(options.scale, fit),
      });
...
async function fetchIiifImage(
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (res.status !== 429 || attempt === IIIF_RETRY_WAITS.length) {
      throw new Error(
        relayFailure(res, `Could not fetch the IIIF image at ${url}`),
      );
    }
...
export async function prepareCandidates(
  ...
  for (const file of files) {
    ...
    if (!kind) {
      throw new Error(
        `Unsupported file: ${file.name}. Upload JPG, PNG, PDF, MEI or MusicXML.`,
      );
    }
    ...
    if (kind === "pdf") {
      ...
      const rendered = await renderPdf(file, {
        ...
        onPage: (page, pages) =>
          onProgress?.({ detail: `page ${page} of ${pages}` }),
      });
  ...
  if (iiifCanvases.length) {
    ...
    const previews = await fetchInParallel(
      iiifCanvases,
      (canvas) =>
        fetchIiifImage(canvas, previewEdge, options.brokerUrl!, doFetch, pacer),
      (fetched) =>
        onProgress?.({ detail: `${fetched} of ${iiifCanvases.length}` }),
    );
    ...
    for (const [i, preview] of previews.entries()) {
      await addCandidate(preview, `Canvas ${i + 1}`, {
        kind: "iiif",
        canvas: iiifCanvases[i],
      });
    }
  }
...
export async function resolvePages(
  ...
  for (const [file, numbers] of wanted) {
    ...
    onProgress?.({
      step: `Rendering ${ordered.length} page(s) of ${file.name}`,
    });
    const blobs = await renderPdf(file, {
      scale: PDF_RENDER_SCALE,
      pages: ordered,
      onPage: (page, count) =>
        onProgress?.({ detail: `page ${page} of ${count}` }),
    });
  ...
  for (const page of pages) {
    ...
    if (page.source.kind === "pdf-page") {
      ...
      if (!page.source.canvas.service && raster.type === "image/png")
        extension = "png";
    }
    images.push({
...
export async function fetchIiifCanvases(
  ...
  if (!res.ok) {
    throw new Error(
      relayFailure(res, `Could not fetch the IIIF manifest at ${manifestUrl}`),
    );
  }
  ...
  if (!canvases.length)
    throw new Error("No canvas images were found in that IIIF manifest.");
  return canvases;
}
