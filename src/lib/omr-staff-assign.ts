...
function labelParts(text: string): LabelParts {
  const tokens = text
    ...
    .replace(
      /(^|[^a-z0-9])[il1|]+(?=$|[^a-z0-9])/g,
      (m, lead: string) => lead + "i".repeat(m.length - lead.length),
    )
...
function partsScore(read: LabelParts, label: string): number {
  ...
  if (
    !read.words.some(
      (r) => r.length >= 2 && words.some((w) => abbreviates(r, w)),
    )
  )
    return 0;
...
function readLabel(printed: string, labels: string[]): LabelParts {
  ...
  for (const word of read.words) {
    ...
    const tail =
      run.length === 1 && labelWords.some((w) => abbreviates(word, w))
        ? ""
        : run;
    let choice = { stem: word, extra: [] as string[], rank: [-1, 0] };
    for (
      let k = 0;
      k <= tail.length && word.length - k >= 3 - (k ? 0 : 1);
      k++
    ) {
      ...
      const better =
        rank[0] > choice.rank[0] ||
        (rank[0] === choice.rank[0] && rank[1] >= choice.rank[1]);
...
export function suggestStaffAssignment(
  ...
  const refGaps =
    reference && reference.length === m ? relativeGaps(reference) : null;
  const labels = parts.map((part) => part.label);
  const readings = printed.map((staff) =>
    staff.label ? readLabel(staff.label, labels) : null,
  );
