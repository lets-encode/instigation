  ...
  return {
    ...
    validation: {
      required_validations:
        fields.required_validations ?? DEFAULTS.required_validations,
  ...
  const pieces = config.pieces
    .map((piece) => {
      ...
      return (
        ...
        (piece.preparation
          ? `    preparation: ${yamlStr(piece.preparation)}\n`
          : "") +
...
export function planTasks(
  ...
  for (const piece of config.pieces) {
    ...
    const locator =
      piece.preparation === "omr" ? "omr-layout" : "measure-zones";
 ...
 */
export function buildTaskCsv(
  config: CampaignConfig,
  surfaces?: PieceSurfaces,
): string {
  const lines = [csvRow(TASK_COLUMNS)];
  for (const { id, fragment, locator, dependsOn } of planTasks(
    config,
    surfaces,
  )) {
 ...
 */
export function buildStateCsv(
  config: CampaignConfig,
  surfaces?: PieceSurfaces,
): string {
  const count = config.validation?.required_validations ?? 0;
  const validationCols = Array.from(
    { length: count },
    (_, i) => `validate_status_${i + 1}`,
  );
