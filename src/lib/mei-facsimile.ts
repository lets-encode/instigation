...
//
// A facsimile campaign's score.mei passes through three stages, all emitted by
// buildFacsimileMei from one model:
//   A. facsimile-only  — <surface>/<graphic> per page, one labelled
//      <zone type="measure" n="…"> per measure box, empty <section>. Committed
//      at init; the measure-correction pre-task rewrites the zones.
//   B. + measures      — one <measure n="label" facs="#zone"> (holding an
//      <mRest/>) per zone. Generated by the coordinator when the correction
//      task's validation completes.
//   C. + breaks        — a <pb/> before each page's first measure and an
//      <sb/> before each measure flagged as starting a system. Written by the
//      breaks pre-task.
...
/** One measure zone: its box, its measure label (@n), and its system flag. */
...
export function buildFacsimileMei(
	...
	model.pages.forEach((page, pi) => {
		...
		page.zones.forEach((zone, zi) => {
			...
			if (withBreaks && zone.sb && zi > 0) {
				sectionParts.push(`               <sb xml:id="sb-${p}-${zi + 1}"/>`);
			}
			sectionParts.push(
...
export function parseFacsimileMei(text: string): ParsedFacsimile {
	...
	// Which zones the section marks as system/page starts (stage C), keyed by
	// zone id via each measure's facs link.
	const section = /<section>([\s\S]*?)<\/section>/.exec(text)?.[1] ?? '';
	const sbZones = new Set<string>();
	let hasMeasures = false;
	let hasBreaks = false;
	let pendingBreak = false;
	for (const token of section.match(/<(pb|sb|measure)\b[^>]*>/g) ?? []) {
		if (token.startsWith('<pb') || token.startsWith('<sb')) {
			pendingBreak = true;
			if (token.startsWith('<pb')) hasBreaks = true;
		} else {
			hasMeasures = true;
			const facs = attr(token, 'facs');
			if (pendingBreak && facs?.startsWith('#')) sbZones.add(facs.slice(1));
			pendingBreak = false;
		}
	}
	...
	for (const surfaceMatch of text.matchAll(/<surface\b[^>]*>([\s\S]*?)<\/surface>/g)) {
		...
		for (const zoneMatch of body.matchAll(/<zone\b[^>]*>/g)) {
			...
			zones.push({ box, label, sb: sbZones.has(id) });
			boxesOnly.push(box);
		}
		if (!hasBreaks) {
