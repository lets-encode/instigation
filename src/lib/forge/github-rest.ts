...
function apiRoot(token?: string): string {
  if (token === SESSION) {
    if (!sessionApiBase)
      throw new Error("SESSION used before routeSessionVia() was called.");
...
function endpointFrom(input: RequestInfo | URL): string {
  try {
    ...
    return proxyIndex >= 0
      ? path.slice(proxyIndex + proxyMarker.length) || "/"
      : path;
  } catch {
    return "(invalid URL)";
  }
...
function recordRequest(
  ...
  requestTelemetry.byMethod[method] =
    (requestTelemetry.byMethod[method] ?? 0) + 1;
  const bucket = resource ?? "unknown";
  requestTelemetry.byResource[bucket] =
    (requestTelemetry.byResource[bucket] ?? 0) + 1;
...
const etagCache = new Map<
  string,
  { etag: string; body: string; link: string | null }
>();
...
async function ghSend<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `${detail.message || detail.error || "GitHub API error"} (${res.status} ${method} ${path})`,
    );
  }
  return data as T;
}
...
const repoHeadInfoCache = new Map<
  string,
  { value: { branch: string; canPush: boolean }; expires: number }
>();

function memoGet<K, T>(
  cache: Map<K, { value: T; expires: number }>,
  key: K,
): T | undefined {
...
export async function getRepoById(
  ...
  const { status, ok, data } = await ghGet<{
    ...
    owner?: { login?: string };
    message?: string;
  }>(`${apiRoot(token)}/repositories/${id}`, token);
  if (status === 404) return null;
  if (!ok)
    throw new Error(data?.message || `Failed to resolve repository ${id}`);
...
export async function createRepoFromTemplate(
  ...
  const res = await githubFetch(
    `${apiRoot(token)}/repos/${templateOwner}/${templateRepo}/generate`,
    {
      method: "POST",
      headers: { ...baseHeaders, ...authHeaders(token) },
      body: JSON.stringify({
...
export async function getRepoFile(
  ...
  if (blob === null)
    throw new Error(`Failed to fetch ${path}: content unavailable`);
...
export async function getRepoFileDownloadUrl(
  ...
  if (status === 404) {
    console.log("[getRepoFileDownloadUrl] path or ref absent", {
      owner,
      repo,
      path,
      ref,
  ...
  if (!ok) throw new Error(data?.message || `Failed to fetch ${path}`);
  console.log("[getRepoFileDownloadUrl] temporary URL available", {
    path,
    ref: ref ?? "(default)",
    status,
  });
...
export async function getRepoHead(
  ...
  if (!info) {
    const repoRes = await ghGet<RepoData>(
      `${apiRoot(token)}/repos/${owner}/${repo}`,
      token,
    );
    if (!repoRes.ok)
      throw new Error(
        `${repoRes.data?.message || "Failed to read repository"} (${repoRes.status} GET repo)`,
      );
    info = {
      branch: repoRes.data.default_branch,
      canPush: Boolean(repoRes.data.permissions?.push),
    };
    repoHeadInfoCache.set(key, {
      value: info,
  ...
  const branchRes = await ghGet<{
    ...
    message?: string;
  }>(`${apiRoot(token)}/repos/${owner}/${repo}/branches/${info.branch}`, token);
  if (!branchRes.ok)
    throw new Error(
      `${branchRes.data?.message || "Failed to read branch"} (${branchRes.status} GET branches/${info.branch})`,
    );
...
export async function ensureFork(
  ...
  if (known) {
    await ghSend(
      "POST",
      `/repos/${known.owner}/${known.repo}/merge-upstream`,
      token,
      { branch: known.branch },
    );
  ...
  for (let i = 0; i < attempts; i++) {
    const r = await githubFetch(
      `${apiRoot(token)}/repos/${forkOwner}/${forkRepo}/git/ref/heads/${data.default_branch}`,
      { headers, cache: "no-store" },
    );
    if (r.ok) {
      await ghSend(
        "POST",
        `/repos/${forkOwner}/${forkRepo}/merge-upstream`,
        token,
        {
          branch: data.default_branch,
        },
      );
      knownForks.set(`${owner}/${repo}`, {
        owner: forkOwner,
        repo: forkRepo,
        branch: data.default_branch,
      });
...
export async function repoExists(
  ...
  const { status, ok, data } = await ghGet<ErrorResponse>(
    `${apiRoot(token)}/repos/${owner}/${repo}`,
    token,
  );
...
export async function getRepoInfo(
  ...
  const { status, ok, data } = await ghGet<RepoData>(
    `${apiRoot(token)}/repos/${owner}/${repo}`,
    token,
  );
  if (status === 404) return null;
...
export async function getRepoAccess(
  ...
  const { ok, data } = await ghGet<RepoData>(
    `${apiRoot(token)}/repos/${owner}/${repo}`,
    token,
  );
  if (!ok) throw new Error(data?.message || "Failed to read repository");
...
async function pullRequestFilesPage(
  ...
  return (data as PullRequestFile[]).map((f) => ({
    filename: f.filename,
    status: f.status,
    patch: f.patch,
  }));
...
export async function getPullRequestFiles(
  ...
  if (expectedChangedFiles > 3000) {
    throw new Error(
      "Pull request changes more than GitHub’s 3,000-file inspection limit.",
    );
  }
  ...
  if (files.length !== expectedChangedFiles) {
    throw new Error(
      `Incomplete pull-request file list: expected ${expectedChangedFiles}, received ${files.length}.`,
    );
  }
  return files;
}
...
async function commitFilesOnBase(
  ...
  }>("GET", `/repos/${owner}/${repo}/commits/${sha}`, token);
  if (data.parents?.length !== 1 || data.parents[0].sha !== baseSha)
    return null;
  ...
  if (files.length !== expectedChangedFiles) {
    throw new Error(
      `Incomplete commit file list: expected ${expectedChangedFiles}, received ${files.length}.`,
    );
  }
  return files;
}
...
export async function getPullRequest(
  ...
  if (firstPage instanceof Error) {
    if (firstPage instanceof RateLimitError || details.commits !== 1)
      throw firstPage;
...
export async function listOpenPullRequests(
  ...
  const { ok, data } = await ghGet<
    Array<OpenPullRequest> | { message?: string }
  >(
  ...
  if (!ok || !Array.isArray(data)) {
    throw new Error(
      (data as { message?: string })?.message || "Failed to list pull requests",
    );
  }
  return data.map((pr) => ({
    number: pr.number,
...
export async function getLastIssueComment(
  ...
  if (!probe.ok)
    throw new Error(
      (probe.data as unknown as ErrorResponse)?.message ||
        "Failed to read comments",
    );
  const lastPage = probe.link?.match(
    /<[^>]*[?&]page=(\d+)[^>]*>;\s*rel="last"/,
  )?.[1];
  if (!lastPage)
    return (Array.isArray(probe.data) ? probe.data : [])[0]?.body ?? null;
  ...
  if (!ok)
    throw new Error(
      (data as unknown as ErrorResponse)?.message || "Failed to read comments",
    );
...
export async function closePullRequest(
  ...
  await ghSend("PATCH", `/repos/${owner}/${repo}/pulls/${number}`, token, {
    state: "closed",
  });
}
...
export async function waitForRepoContents(
  ...
  throw new Error(
    `Timed out waiting for ${path} to appear in ${owner}/${repo}.`,
  );
}
...
export async function commitFiles(
  ...
  if (newBranch && !baseSha)
    throw new Error("newBranch requires baseSha (the commit to branch from).");
  if (baseTreeSha && !baseSha)
    throw new Error("baseTreeSha requires baseSha (the commit it belongs to).");
  for (const file of files) {
    const hasText = file.content != null;
    const hasBinary = file.contentBase64 != null;
    if (hasText === hasBinary) {
      throw new Error(
        `File ${file.path} must provide exactly one of content or contentBase64.`,
      );
    }
  }
  ...
  const targetBranch =
    newBranch ??
    branch ??
    (await ghSend<{ default_branch: string }>("GET", api, token))
      .default_branch;
  let headSha = baseSha;
  if (!headSha) {
    const ref = await ghSend<{ object: { sha: string } }>(
      "GET",
  ...
  const tree = await ghSend<{ sha: string }>(
    "POST",
    `${api}/git/trees`,
    token,
    {
  ...
  if (newBranch) {
    await ghSend("POST", `${api}/git/refs`, token, {
      ref: `refs/heads/${newBranch}`,
      sha: commit.sha,
    });
  } else {
    await ghSend("PATCH", `${api}/git/refs/heads/${targetBranch}`, token, {
      sha: commit.sha,
    });
  }
...
export async function fastForwardBranch(
  ...
  const res = await githubFetch(
    `${apiRoot(token)}/repos/${owner}/${repo}/git/refs/heads/${branch}`,
    {
      method: "PATCH",
      headers: { ...baseHeaders, ...authHeaders(token) },
  ...
  if (
    res.status === 409 ||
    (res.status === 422 && /fast.?forward/i.test(data.message ?? ""))
  ) {
    return false;
  }
  throw new Error(
    `${data.message || "Failed to fast-forward branch"} (${res.status} PATCH ref heads/${branch})`,
  );
}
...
export async function deleteBranch(
  ...
  if (
    res.status === 422 &&
    /reference does not exist/i.test(data.message ?? "")
  )
    return;
  throw new Error(
    `${data.message || "Failed to delete branch"} (${res.status} DELETE ref heads/${branch})`,
  );
}
...
export async function createPullRequest(
  ...
  const res = await githubFetch(
    `${apiRoot(token)}/repos/${owner}/${repo}/pulls`,
    {
      method: "POST",
      headers: { ...baseHeaders, ...authHeaders(token) },
...
export async function openChangePr(
  ...
  const target = canPush
    ? { owner, repo }
    : await ensureFork(token, owner, repo);
  ...
  try {
    await commitFiles(token, target.owner, target.repo, files, message, {
      baseSha: sha,
      newBranch: branch,
    });
    branchCreated = true;
    ...
    if (branchCreated) {
      try {
        ...
        console.warn(
          `Could not clean up branch ${target.owner}/${target.repo}:${branch}: ${(cleanupError as Error).message}`,
        );
      }
    }
    throw error;
...
export async function setRepoTopics(
  ...
  return ghSend<unknown>("PUT", `/repos/${owner}/${repo}/topics`, token, {
    names,
  });
}
 ...
 */
export async function setActionsWorkflowPermissions(
  token: string,
  owner: string,
  repo: string,
): Promise<void> {
...
export async function getRepoSubscription(
  ...
  const { status, ok, data } = await ghGet<{
    ...
    ignored?: boolean;
    message?: string;
  }>(`${apiRoot(token)}/repos/${owner}/${repo}/subscription`, token, {
    cache: false,
  });
  if (status === 404) return null;
  if (!ok)
    throw new Error(data?.message || "Failed to read repository subscription");
  return {
 ...
 * own user.
 */
export async function ignoreRepoNotifications(
  token: string,
  owner: string,
  repo: string,
): Promise<void> {
  await ghSend("PUT", `/repos/${owner}/${repo}/subscription`, token, {
    subscribed: false,
    ignored: true,
  });
}
