...
async function openClaimPr(token, owner, repo, user, task_id, kind) {
...
export async function load({ params, locals }) {
	...
	let stateCsv, locksCsv, isPrivate;
	try {
		...
		throw error(502, `Could not read ${owner}/${repo}: ${e.message}`);
	}
...
export const actions = {
	...
	claim: async ({ params, request, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Claim failed: ${e.message}` });
		}
	},
	...
	editor: async ({ params, request, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Open in mei-friend failed: ${e.message}` });
		}
	},
	...
	rawlink: async ({ params, request, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Raw link failed: ${e.message}` });
		}
	},
	...
	encode: async ({ params, request, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Encode failed: ${e.message}` });
		}
	},
	...
	validate: async ({ params, request, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Validate failed: ${e.message}` });
		}
	},
	...
	reaper: async ({ params, locals }) => {
		...
		try {
			...
			return fail(502, { error: `Reaper dispatch failed: ${e.message}` });
		}
	}
};
