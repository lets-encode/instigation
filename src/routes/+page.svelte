<script lang="ts">
  ...
  const selectedLicense = $derived(
    LICENSES.find((l) => l.id === license) ?? LICENSES[0],
  );
  ...
  $effect(() => {
    ...
    const timer = setTimeout(async () => {
      const clash = campaignRepos.find(
        (r) => r.name.toLowerCase() === h.toLowerCase(),
      );
      if (clash) {
    ...
    for (const span of [3, 2]) {
      ...
      const note =
        modeWord === "dur" || modeWord === "moll"
          ...
          : (SOLFEGE_NOTES[tokens[i]] ??
            (/^[a-g]$/.test(tokens[i]) ? tokens[i] : undefined));
      if (!note) continue;
  ...
  const CATALOGUE_KEEP_NUMBER = new Set([
    "no",
    "nr",
    "num",
    "number",
  ...
  async function createCampaign(e: SubmitEvent) {
    ...
    if (sourceMode === "facsimile" && sourceFiles.length === 0) {
      return void (error =
        "Add at least one page image or a PDF, or choose a different source.");
    }

    submitting = true;
    console.log("[create] creating campaign", {
      title: t,
      handle: h,
      sourceMode,
      visibility,
    ...
    if (sourceMode === "facsimile") {
      ...
      if (facsimile.pages.every((p) => p.measures.length === 0)) {
        error =
          "No measures were detected on the uploaded pages. Check the images and try again.";
        submitting = false;
        progress = null;
        return;
      }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Could not tag new repo with topic:",
      ...
      const initialise = async () => {
        const template = await f.waitForRepoContents(
          owner,
          repo.name,
        ...
        if (facsimile) {
          ...
          mei = buildFacsimileMei(
            initialFacsimileModel(facsimile.pages, header),
          );
          const { blobToBase64 } = await import("$lib/facsimile-detect.ts");
          for (const img of facsimile.images) {
            imageFiles.push({
              path: img.path,
              contentBase64: await blobToBase64(img.blob),
            });
          }
        } else {
        ...
        // One encoding task per page carrying measures (facsimile only).
        const pageMeasureCounts = facsimile?.pages.map(
          (p) => p.measures.length,
        );
        ...
        await f.commitFiles(
          ...
          [
            ...
            ...imageFiles,
            {
              ...
              content: buildTaskCsv(config, pageMeasureCounts),
            },
            {
              path: "tracking/state.csv",
              content: buildStateCsv(config, pageMeasureCounts),
            },
        ...
        console.log(
          "[create] campaign initialised: committed config, score and tracking tables",
        );
      };
      try {
        await initialise();
        ...
        console.error(
          "Campaign initialisation failed:",
          (err as Error).message,
        );
        created = {
      ...
      if (facsimile && facsimile.skipped.length) {
        console.warn(
          "[create] pages skipped (detector error):",
      ...
      error =
        "Could not create the repository. Check the handle isn’t already taken, then try again.";
      submitting = false;
      progress = null;
    }
  }
...
{#if submitting}
  <div class="overlay" role="status" aria-live="polite">
    <div class="overlay-card">
      ...
      <p class="overlay-sub">
        {progress ?? "Setting up the campaign files. This takes a few seconds."}
      </p>
    </div>
  </div>
{/if}
...
<section class="hero">
  ...
  <p>
    Log in with GitHub and fill in your campaign details. We'll create a GitHub
    repository from a template — it holds your campaign's score, configuration
    and progress.
  </p>
...
{#if auth.user && showForm}
  <section class="create">
    ...
    {#if created}
      <div class="banner ok">
        Created <a href={created.html_url} target="_blank" rel="noreferrer"
          >{created.full_name}</a
        > 🎉
      </div>
      {#if created.initWarning}
        <div class="banner warn">
          The repository was created, but setting up its campaign files didn't
          finish.
          <button type="button" class="linkish" onclick={retryCreatedCampaign}
            >Retry initialisation</button
          >
    ...
    <form onsubmit={createCampaign}>
      ...
      <label>
        ...
        />
        <span class="hint"
          >Used in the URL and as the Git repository name. Auto-filled from the
          campaign name — edit it if you like.</span
        >
        ...
        {:else if handleCheck.state === "unknown"}
          <span class="hint"
            >Couldn't check availability — it will be verified when the
            repository is created.</span
          >
        {/if}
      </label>
      ...
      <fieldset class="source">
        ...
        <label class="radio">
          ...
          <span>
            Page images or PDF
            <span class="opt-hint"
              >Detect measures and scaffold an empty score linked to the pages.</span
            >
          </span>
        </label>
        ...
        {#if sourceMode === "facsimile"}
          ...
          >
            <span class="drop-label"
              >Drag &amp; drop files here, or pick them:</span
            >
            ...
            <span class="hint">
              {#if sourceFiles.length}
                ...
                Each PDF is split into one image per page; all pages are combined
                into one score in the order shown.
              {:else}
                One or more PDFs (split into one image per page) and/or JPG/PNG
                page images.
              {/if}
            </span>
            {#if dropNote}
            ...
            {/if}
            <span class="hint">
              Page images are sent to <a
                href={measureDetectorUrl}
                target="_blank"
                rel="noreferrer">the configured measure detector</a
              >
        ...
        <label class="radio">
          ...
          <span>
            ...
            <span class="opt-hint"
              >Start from a blank one-measure template.</span
            >
          </span>
        </label>
      ...
      <label>
        ...
        <input
          ...
          placeholder="Describe your encoding project in a few words."
        />
      </label>
      ...
      <label>
        ...
        <span class="hint">
          ...
          <a href={selectedLicense.url} target="_blank" rel="noreferrer"
            >Full license text →</a
          >
        </span>
      </label>
...
<section class="repos">
  ...
  {:else}
    ...
    {#if !auth.user}
      <p class="muted small">
        Log in to also see your own private repositories here.
      </p>
    {/if}
  {/if}
</section>
