...
- **OAuth session broker.** The Flask service in `broker/` runs the whole OAuth
  ...
  can read the token. It must be mounted under the SPA's own origin (`/oauth`)
...
- **Authorization callback URL:** `http://localhost:5173/oauth/authorize`
...
the broker at `PUBLIC_OAUTH_BROKER_URL` (default `/oauth`, proxied to the broker
...
The Vite dev server proxies `/oauth` to it, so SPA and broker share an origin.
...
- **SPA + broker, one origin:** the broker's session cookie must be first-party,
  ...
  `/oauth/` to the broker (gunicorn) behind **HTTPS**.
- **Broker env:** `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, `FLASK_SECRET`,
  and `REDIRECT_URL=https://<your-origin>/oauth/authorize` (see `broker/README.md`).
- **OAuth App:** update its callback URL to `<your-origin>/oauth/authorize` (or
