...
- Build output moves to `website/`, the deployed document root; the project website (refreshed from lets-encode.github.io, start form kept) moves into `static/` and is copied into it by the build. `deploy/` (release script and Apache vhosts) removed; the web server requirements are listed in README §6.
