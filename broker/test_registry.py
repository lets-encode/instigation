...
class RegistryTest(unittest.TestCase):
    ...
    def test_register_is_idempotent_for_same_repo(self):
        self.assertEqual(
            self.register("same-name", repo_id=42).status_code, 201
        )
    ...
    def test_claim_then_register_with_token(self):
        ...
        self.assertEqual(r.status_code, 201)
        self.assertEqual(
            self.lookup("held-name").get_json()["status"], "active"
        )
    ...
    def test_release_frees_a_claimed_name(self):
        ...
        self.assertEqual(
            self.lookup("second-thoughts").get_json()["status"], "pending"
        )
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            self.lookup("second-thoughts").get_json()["status"], "free"
        )
    ...
    def test_taken_over_claim_can_no_longer_activate(self):
        ...
        self.assertEqual(
            self.claim("contested").status_code, 201
    ...
    def test_claim_register_and_release_require_the_session(self):
        ...
        self.assertEqual(
            self.register("logged-out", repo_id=8).status_code, 401
        )
        r = self.client.delete(
            "/registry/claim/mine", json={"claim_token": token}
        )
    ...
    def test_register_verifies_push_permission_with_the_sessions_token(self):
        self.assertEqual(
            self.register("owned-name", repo_id=555).status_code, 201
        )
    ...
    def test_tombstone_prevents_reregistration(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            self.lookup("doomed-name").get_json()["status"], "tombstoned"
        )
    ...
    def test_admin_requires_token(self):
        self.assertEqual(
            self.client.get("/registry/admin/slugs").status_code, 401
        )
        self.assertEqual(
            self.client.get(
                "/registry/admin/slugs",
